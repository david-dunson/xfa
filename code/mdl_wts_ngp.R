...
## R CMD BATCH --no-save --no-restore "--args mtd id" mdl_wts_ngp.R mdl_wts_ngp_mtd_id.rout
...
if (mtd == 5) {
    ...
    obj <- readRDS(paste0("../result/ngp/xfa_n_5000_cv_", cid, "_p_", ndim[did], ".rds"))
    fname <- paste0(" ../result/ngp/mdl_wts_n_5000_cv_", cid, "_p_", ndim[did], ".rds")
} else if (mtd == 4) {
    ## n = 500
    ndim <- c(50, 100, 250)
    cvs <- rep(1:10, each = 3)
    ...
    obj <- readRDS(paste0(" ../result/ngp/xfa_n_500_cv_", cid, "_p_", ndim[did], ".rds"))
    fname <- paste0(" ../result/ngp/mdl_wts_n_500_cv_", cid, "_p_", ndim[did], ".rds")
} else if (mtd == 3) {
    ## n = 250
    ndim <- c(50, 100)
    cvs <- rep(1:10, each = 2)
    ...
    obj <- readRDS(paste0(" ../result/ngp/xfa_n_250_cv_", cid, "_p_", ndim[did], ".rds"))
    fname <- paste0(" ../result/ngp/mdl_wts_n_250_cv_", cid, "_p_", ndim[did], ".rds")
} else {
    ## n = 100
    cvtrain <- readRDS("../data/n100.rds")
    train <- cvtrain[[id]]

    obj <- readRDS(paste0(" ../result/ngp/xfa_n_100_cv_", id, "_p_50.rds"))
    fname <- paste0(" ../result/ngp/mdl_wts_n_100_cv_", id, "_p_50.rds")
}
...
mdlMat <- matrix(0.0, 50, 20) ## nalpha <- 20; neta = 50 in xfa_ngp.R
for (aa in 1:20) {
    for (ee in 1:50) {
