...
## mtd = 5 => n = 5000 & p = (50, 100, 250, 500, 2000) depends on id = 1 ... 50
##     = 4 => n = 500  & p = (50, 100, 250) depends on id = 1 ... 30
##     = 3 => n = 250  & p = (50, 100) depends on id = 1 ... 20
##     = 2 => n = 100  & p = 50 & id = 1 ... 10
## id  determines which replication and combination of (n, p)

## provide the (mtd, id) combination via command line
## for example, replace mtd and id below by 5 and 1 to run the analysis for n = 5000, p = 50, and rep = 1
## R CMD BATCH --no-save --no-restore "--args mtd id" competitors_ngp.R comp_ngp_mtd_id.rout
...
## 2. change the 'mtd' and 'id' appropriately to loadthe  correct data set
...
if (mtd == 5) {
    ...
    cvtrain <- readRDS("../data/n5000.rds")
    train <- cvtrain[[cid]][[did]]

    fname1 <- paste0("../result/ngp/fanc_n_5000_cv_", cid, "_p_", ndim[did], ".rds")
    fname2 <- paste0("../result/ngp/spca_n_5000_cv_", cid, "_p_", ndim[did], ".rds")
    fname3 <- paste0("../result/ngp/rg_n_5000_cv_", cid, "_p_", ndim[did], ".rds")
    fname4 <- paste0("../result/ngp/rgp_n_5000_cv_", cid, "_p_", ndim[did], ".rds")
} else if (mtd == 4) {
    ndim <- c(50, 100, 250)
    cvs <- rep(1:10, each = 3)
    ...
    cvtrain <- readRDS("../data/n500.rds")
    train <- cvtrain[[cid]][[did]]

    fname1 <- paste0("../result/ngp/fanc_n_500_cv_", cid, "_p_", ndim[did], ".rds")
    fname2 <- paste0("../result/ngp/spca_n_500_cv_", cid, "_p_", ndim[did], ".rds")
    fname3 <- paste0("../result/ngp/rg_n_500_cv_", cid, "_p_", ndim[did], ".rds")
    fname4 <- paste0("../result/ngp/rgp_n_500_cv_", cid, "_p_", ndim[did], ".rds")
} else if (mtd == 3) {
    ndim <- c(50, 100)
    cvs <- rep(1:10, each = 2)
    ...
    cvtrain <- readRDS("../data/n250.rds")
    train <- cvtrain[[cid]][[did]]

    fname1 <- paste0("../result/ngp/fanc_n_250_cv_", cid, "_p_", ndim[did], ".rds")
    fname2 <- paste0("../result/ngp/spca_n_250_cv_", cid, "_p_", ndim[did], ".rds")
    fname3 <- paste0("../result/ngp/rg_n_250_cv_", cid, "_p_", ndim[did], ".rds")
    fname4 <- paste0("../result/ngp/rgp_n_250_cv_", cid, "_p_", ndim[did], ".rds")
} else {
    cvtrain <- readRDS("../data/n100.rds")
    train <- cvtrain[[id]]

    fname1 <- paste0("../result/ngp/fanc_n_100_cv_", id, "_p_50.rds")
    fname2 <- paste0("../result/ngp/spca_n_100_cv_", id, "_p_50.rds")
    fname3 <- paste0("../result/ngp/rg_n_100_cv_", cid, "_p_", ndim[did], ".rds")
    fname4 <- paste0("../result/ngp/rgp_n_100_cv_", cid, "_p_", ndim[did], ".rds")
}
