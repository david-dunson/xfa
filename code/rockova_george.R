## Code provided by Prof. Veronika Rockova  <Veronika.Rockova@chicagobooth.edu> on 07/25/16
## Implements Algorithm in Table 1 of Rockova & George (2016).
## Rockova & George (2016). Fast Bayesian factor analysis via automatic rotations to sparsity. Journal of the American Statistical Association 111, 1608–1622.
...
M_theta_NLP<-function(P_star,alpha){
    ...
    x0<-sort(rbeta(K,1,1),decreasing=T)
...
FACTOR_ROTATE<-function(Y,lambda0,lambda1,start,K,epsilon,alpha,PX,approximate,stop,varimax){
...
while(eps>epsilon&niter<=stop){
    ...
    P_star                      <-E_step_gamma(B_k,theta_k,lambda0,lambda1)
        ...
        o                                       <-order(apply(P_star,2,sum),decreasing=T)
