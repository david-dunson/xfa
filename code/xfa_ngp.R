...
## R CMD BATCH --no-save --no-restore "--args mtd id" xfa_ngp.R xfa_ngp_mtd_id.rout
...
if (mtd == 5) {
    ...
    fname <- paste0("../result/ngp/xfa_n_5000_cv_", cid, "_p_", ndim[did], ".rds")
} else if (mtd == 4) {
    ndim <- c(50, 100, 250)
    cvs <- rep(1:10, each = 3)
    ...
    fname <- paste0("../result/ngp/xfa_n_500_cv_", cid, "_p_", ndim[did], ".rds")
} else if (mtd == 3) {
    ndim <- c(50, 100)
    cvs <- rep(1:10, each = 2)
    ...
    fname <- paste0("../result/ngp/xfa_n_250_cv_", cid, "_p_", ndim[did], ".rds")
} else {
    cvtrain <- readRDS("../data/n100.rds")
    train <- cvtrain[[id]]

    fname <- paste0("../result/ngp/xfa_n_100_cv_", id, "_p_50.rds")
}
...
nfactor <- 20; npolish <- 50; nalpha <- 20; neta <- 50; tol <- 1e-6
...
alphaMax <- 10 # based on preliminary glmnet runs.
alphaMin <- 2
alphaSeq <- seq(log10(alphaMin), log10(alphaMax), length = nalpha)
etaSeq <- rev(seq(log10(0.01), log10(1e7), length = neta))
...
for (aa in 1:length(alphaSeq)) {
    for (ee in 1:length(etaSeq)) {
        ...
        for (pp in 1:npolish) {
            ...
            penFac <- ((aseq + 1) / as.numeric(10^(etaSeq[ee]) + t(abs(warmLambda)))) * 1 / (nobs * ndim * xfactor)
            ...
            if (class(tryfit)[2] == "glmnet") {
                ...
                ## have we converged?
                obj[pp + 1] <- objVal(warmLambda, lam1, sig1, psiMat, lambdaHatMat, (10^(alphaSeq[aa]))^(1:xfactor),
                                      rep(10^(etaSeq[ee]), xfactor), nobs)
